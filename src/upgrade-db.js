...
const RollupDB = require("./rollup-db");
...
class UpgradeDb {
    ...
    constructor(rollupDb, nAccounts, nLevels) {
        ...
        this.dbState = new SMTTmpDb(this.rollupDb.db);
        this.stateTree = new SMT(this.dbState, this.rollupDb.stateRoot);
        this.currentBatch = this.rollupDb.lastBatch + 1;
    }
    ...
    async doUpgrade() {
        ...
        const insertsState = Object.keys(this.dbState.inserts).reverse().map((key) => {
            return [Scalar.e(key), this.dbState.inserts[key]];
        });

        await this.rollupDb.db.multiIns([
            ...insertsState,
            [Scalar.add(Constants.DB_Batch, this.currentBatch), this.stateTree.root],
            [Scalar.add(Constants.DB_InitialIdx, this.currentBatch), this.maxIdx],
    ...
    async _buildSingleProof(initKey, finalKey) {
        ...
        for (let i = initKey; i < finalKey; i++) {
            ...
            if (i <= this.maxIdx) {
                ...
                const lastIdStates = await this.dbState.get(Scalar.add(Constants.DB_Idx, i));
                // add last batch number
                let valStatesId;
                if (!lastIdStates) valStatesId = [];
                else valStatesId = [...lastIdStates];
                if (!valStatesId.includes(this.currentBatch)) valStatesId.push(this.currentBatch);

                // new state for idx
                const newValueId = poseidonHash([newValue, i]);

                // new entry according idx and batchNumber
                const keyIdBatch = poseidonHash([i, this.currentBatch]);

                await this.dbState.multiIns([
                    [newValueId, stateUtils.state2Array(newState)],
                    [keyIdBatch, newValueId],
                    [Scalar.add(Constants.DB_Idx, i), valStatesId],
                ]);
